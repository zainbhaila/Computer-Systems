   ...
   for (i = 0; i < doc->number_of_paragraphs; i++) {
      for (j = 0; j < doc->paragraphs[i].number_of_lines; j++) {
         ...
         while (found != NULL) {
            ...
            strncat(&copy[0], doc->paragraphs[i].lines[j], (size_t) loc);
...
/*add data lines to document from a file*/
int load_file(Document * doc, const char *filename) {
   ...
   while (fgets(line, MAX_STR_SIZE + 1, file) != NULL) {
      for (i = 0; i < strlen(line); i++) {
         if (!isspace(line[i])) {
            append_line(doc, paragraphs_added, line);
            flag = 0;
            break;
         }
      }
      ...
      /*new paragraph */
      if (flag) {
         if (add_paragraph_after(doc, paragraphs_added) == FAILURE) {
            return FAILURE;
         }
         paragraphs_added++;
      }
